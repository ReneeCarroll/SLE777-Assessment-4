...
```{r}
dna_ecoli <- unlist(ecoli_cds) # 'unlist' takes the nested list of DNA sequences from the 'ecoli_cds' data frame, and saves it as a single vector called dna_ecoli. If you now perform the code summary(dna_ecoli), you see that this vector as a length of 3978528, which is the total number of nucleotides as determined earlier for E.coli in Step 2 above. 
dna_composition_ecoli <- count(dna_ecoli,1) # this counts the number of each nucleotide in the vector 'dna_ecoli', in combinations of 1's (ie. single nucleotides), and saves it as 'dna_composition_ecoli'. The code 'print(dna_composition_ecoli)' will show that there are 955768, 977594, 1088501, and 956665 of a, c, g, t, respectively.
dna_proportion_ecoli <- dna_composition_ecoli/sum(dna_composition_ecoli)
print(dna_proportion_ecoli)
```

```{r}
str(dna_proportion_ecoli)
```
...
barplot(dna_proportion_ecoli, xlab = "Nucleotides", ylab = "Proportion", main = "E.coli coding DNA sequence composition")
grid()
```

```{r bar plots for frequency of amino acids in the total protein sequences for both organisms}
barplot(dna_proportion_aaceti, xlab = "Nucleotides", ylab = "Proportion", main = "A.aceti coding DNA sequence composition")
grid()
```

```{r attempting to create a grouped bar chat, to replace the two single ones above}
...
```{r installl and load ggplot2 and ggpubr}
...
#translate the cDNA from E.coli
#the lapply function performs a loop, applying the 'translate' code to all elements of the 'ecoli_cds' data, and saving the protein sequences to the 'ecoli_prot' object.
...
```{r}
ecoli_prot_total <- unlist(ecoli_prot) # 'unlist' takes the nested list of protein sequences from the 'ecoli_prot' data frame, and saves it as a single vector called ecoli_prot_total. If you now perform the code summary(ecoli_prot_total), you see that this vector has a length of 1326176 characters, which is the total number of nucleotides (3978528, as determined earlier for E.coli in Step 2 above) divided by 3, as expected. 
aa <- unique(ecoli_prot_total) # this creates the list of possible amino acids to form our alphabet of possible options, to use in the 'count' code.
aa <- aa[aa != "*"] # this removes the stop codon asterisks. 
length(aa)
print(aa)
aa_composition_ecoli <- count(ecoli_prot_total, wordsize=1, alphabet=aa) # this counts the number of each nucleotide in the vector 'dna_ecoli', in combinations of 1's (ie. single nucleotides), and saves it as 'dna_composition_ecoli'. The code 'print(dna_composition_ecoli)' will show that there are 955768, 977594, 1088501, and 956665 of a, c, g, t, respectively.
...
```
```{r getting the amino acid composition for A.aceti}
aaceti_prot_total <- unlist(aaceti_prot) # 'unlist' takes the nested list of protein sequences from the 'ecoli_prot' data frame, and saves it as a single vector called ecoli_prot_total. If you now perform the code summary(ecoli_prot_total), you see that this vector has a length of 1326176 characters, which is the total number of nucleotides (3978528, as determined earlier for E.coli in Step 2 above) divided by 3, as expected. 
aa_composition_aaceti <- count(aaceti_prot_total, wordsize=1, alphabet=aa) # this counts the number of each nucleotide in the vector 'dna_ecoli', in combinations of 1's (ie. single nucleotides), and saves it as 'dna_composition_ecoli'. The code 'print(dna_composition_ecoli)' will show that there are 955768, 977594, 1088501, and 956665 of a, c, g, t, respectively.
...
```{r attempting to create a grouped bar chat for amino acid usage}
...
```{r}
str(codons_ecoli_df)
codon_names <- codons_ecoli_df$codon
barplot(codons_ecoli_df$RSCU, xlab="Codons", names.arg = codon_names, las=2)
```

```{r}
test_codons_ecoli_df <- codons_ecoli_df
test_codons_ecoli_df$"RSCU-1" <- test_codons_ecoli_df$RSCU-1
ordered_test_codons_ecoli_df <- test_codons_ecoli_df[order(test_codons_ecoli_df$AA), ]
ordered_test_codons_ecoli_df$"AA+codon" <- paste(ordered_test_codons_ecoli_df$AA, ":", ordered_test_codons_ecoli_df$codon)
print(ordered_test_codons_ecoli_df)
```

```{r}
AAcodon <- ordered_test_codons_ecoli_df$"AA+codon"
barplot(ordered_test_codons_ecoli_df$"RSCU-1", names.arg = AAcodon, las=2)
```
